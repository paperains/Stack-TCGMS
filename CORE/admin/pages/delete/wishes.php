...
if ( isset($_POST['submit']) ) {
    $id = $_POST['id'];
	$getdata = $database->query("SELECT * FROM `user_wishes` WHERE id='$id'");
	$delete = $database->query("DELETE FROM `user_wishes` WHERE id='$id'");
...
if (empty($id)) {
	...
	$getdata = $database->query("SELECT * FROM `user_wishes` WHERE id='$id'");
	echo '<h1>Wishes <span class="fas fa-angle-right" aria-hidden="true"></span> Delete a Wish?</h1>';
	...
	echo '<form method="post" action="index.php?action=delete&page=wishes">
	<input type="hidden" name="id" value="'.$id.'" />
	Are you sure you want to delete this wish? <b>This action can not be undone!</b><br />
