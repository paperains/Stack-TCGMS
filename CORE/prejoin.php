...
if(empty($form)) {
    ...
    <tr><td class="headLine" width="15%">Category</td><td class="headLine" width="25%">Features</td><td class="headLine" width="20%">Set/Series</td><td class="headLine" width="20%">Donator</td></tr>';
    $sql = $database->query("SELECT * FROM `tcg_donations` WHERE `type`='Donations' ORDER BY `category`, `deckname` ASC");
...
###############################
    ...
    if ($act == "sent") {
        if (!isset($_POST['submit']) || $_SERVER['REQUEST_METHOD'] != "POST") { exit("<p>You did not press the submit button; this page should not be accessed directly.</p>"); }
            ...
            $insert = $database->query("INSERT INTO `tcg_donations` (`name`,`category`,`deckname`,`feature`,`set`,`type`,`pass`,`date`) VALUES ('$name','$cat','$deck','$feat','$set','Claims','$pass','$date')");
            
            if ($insert == TRUE) {
                echo '<h1>Deck Claims</h1>
                <p>Your deck claim has been added to the database. You can send the donation link using the <a href="prejoin.php?form=deck-donations">donations</a> form once ready.</p>
                <p>Claim <a href="prejoin.php?form=deck-claims">more?</a></p>';
            } else {
                echo '<h1>Deck Claims : Error</h1>
                <p>There was an error while processing your donations. Kindly send us your donation details instead at <u>'.$tcgemail.'</u>. Thank you and sorry for the inconvenience!</p>';
            }
        }
    } else {
        echo '<h1>Deck Claims</h1>
        ...
        <form method="post" action="prejoin.php?form=deck-claims&action=sent">
        <table width="100%" cellspacing="3" class="border">
            <tr>
                <td class="headLine" width="15%">Name:</td><td class="tableBody"><input type="text" name="name" placeholder="Jane Doe" style="width:90%;"></td>
            ...
            <tr>
                ...
                echo '</td>
                <td class="headLine" width="15%">File Name:</td><td class="tableBody" width="35%"><input type="text" name="deckname" style="width:90%;" placeholder="e.g. whitetigers"></td>
            </tr>
            <tr>
                <td class="headLine">Feature:</td><td class="tableBody"><input type="text" name="feature" placeholder="usually what\'s the deck all about" style="width:90%;"></td>
                <td class="headLine">Set (or series):</td><td class="tableBody"><input type="text" name="set" placeholder="REMOVE if this is not applicable" style="width:90%;"></td>
            </tr>
            <tr><td class="tableBody" align="center" colspan="4"><input type="submit" name="submit" class="btn-success" value="Send Claims" /> <input type="reset" name="reset" class="btn-cancel" value="Reset" /></td></tr>
        </table>
        </form>';
    } // END DONATION FORM
}
...
##################################
    ...
    if ($act == "sent") {
        if (!isset($_POST['submit']) || $_SERVER['REQUEST_METHOD'] != "POST") { exit("<p>You did not press the submit button; this page should not be accessed directly.</p>"); }
            ...
            $update = $database->query("UPDATE `tcg_donations` SET url='$url', type='Donations' WHERE deckname='$deck' AND pass='$pass'");
            
            if ($update == TRUE) {
                echo '<h1>Deck Donations</h1>
                <p>Your deck donation has been received and a deck maker will check it. Once it is approved, you will receive your rewards on your mailbox.</p>
                <p>Donate <a href="prejoin.php?form=deck-donations">more?</a></p>';
            } else {
                echo '<h1>Deck Donations : Error</h1>
                <p>There was an error while processing your donations. Kindly send us your donation details instead at <u>'.$tcgemail.'</u>. Thank you and sorry for the inconvenience!</p>';
            }
        }
    } else {
        ...
        <li>For <b>every deck</b> you donate, you will get X random cards and X CURRENCY.</li></ul>
        <form method="post" action="prejoin.php?form=deck-donations&action=sent">
        <table width="100%" cellspacing="3" class="border">
            <tr>
                <td class="headLine" width="15%">File Name:</td><td class="tableBody"><input type="text" name="deckname" placeholder="e.g. whitetigers" style="width:90%;"></td>
...
##############################
    ...
    if ($act == "sent") {
        if (!isset($_POST['submit']) || $_SERVER['REQUEST_METHOD'] != "POST") { exit("<p>You did not press the submit button; this page should not be accessed directly.</p>"); }
            ...
            if ($now > $yesterday) {
                ...
                if ($update == TRUE) {
                    ...
                    <p>Your votes has been added to the database. You can vote again after 24 hours for the decks you\'ve just voted.</p>';
                } else {
                    echo '<h1>Deck Voting : Error</h1>
                    <p>There was an error while processing your votes. Kindly send us your voting details instead at <u>'.$tcgemail.'</u>. Thank you and sorry for the inconvenience!</p>';
                }
            } else {
                echo '<h1>Deck Voting : Halt!</h1>
                <p>It seems like it hasn\'t been 24 hours since the last time you\'ve sent in a vote. Kindly wait for a couple of more hours before voting again, thank you!</p>';
            }
        }
    } else {
        echo '<h1>Deck Voting</h1>
        ...
        <form method="post" action="prejoin.php?form=votes&action=sent">
        <center><table width="70%" cellspacing="3" class="border">
            <tr>
                <td class="headLine" width="15%">Name:</td><td class="tableBody"><input type="text" name="name" placeholder="Jane Doe" style="width:90%;"></td>
            </tr>';
            for($i=1;$i<=10;$i++) {
                echo '<tr>
                <td class="headLine">Vote '.$i.'</td><td class="tableBody"><select name="deck'.$i.'" style="width:95;%">';
            ...
            echo '<tr><td class="tableBody" align="center" colspan="4"><input type="submit" name="submit" class="btn-success" value="Send Votes" /> <input type="reset" name="reset" class="btn-cancel" value="Reset" /></td></tr>
        </table></center>
        </form>
        <h2>Vote Logs</h2>
